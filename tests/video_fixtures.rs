...
use camwatch_motion_detection::{
    ...
    ffmpeg::{FrameDimensions, StreamOptions, VideoFrame, stream_input_with_options},
    motion::MotionDetector,
    session::MotionSessionTracker,
};
use common::{
    FIXTURE_DIRECTORY, FixtureExpectation, VideoFixture, discover_fixtures, ffmpeg_is_available,
    fixture_test_config,
};
...
fn video_fixtures_match_expected_motion_start() {
    ...
    let fixtures = match discover_fixtures(std::path::Path::new(FIXTURE_DIRECTORY)) {
...
fn assert_fixture(fixture: &VideoFixture) -> Result<(), String> {
    ...
    match (fixture.expectation, first_detection_frame) {
        ...
        (FixtureExpectation::MotionStartsAt(expected_second), Some(frame)) => {
            let earliest_allowed_frame = expected_second
                .saturating_sub(DETECTION_TOLERANCE_SECONDS)
                * u64::from(settings.frame_rate);
            let latest_allowed_frame =
                (expected_second + DETECTION_TOLERANCE_SECONDS) * u64::from(settings.frame_rate);

            if frame < earliest_allowed_frame {
                return Err(format!(
                    "fixture {}: detected too early at frame {} ({}), expected no earlier than second {} (-{}s tolerance)",
                    fixture.path.display(),
                    frame,
                    format_detected_second(detected_second),
                    expected_second,
...
fn run_motion_detection(
    ...
    for frame in frame_receiver {
        ...
        let captures = session_tracker.ingest(frame, analysis);

        if first_detection_frame.is_none()
            && let Some(capture) = captures.first()
        {
            first_detection_frame = Some(capture.motion_started_frame_index);
        }
    }

    if first_detection_frame.is_none()
        && let Some(capture) = session_tracker.finish().first()
    {
        first_detection_frame = Some(capture.motion_started_frame_index);
    }
